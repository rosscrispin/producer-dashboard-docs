...
ui_location: "Settings > Storage (conceptual)"
depends_on:
...
:::note
Mix identifier stripping (`clean`, `full mix`, `demo`, `wip`, etc.) is a planned feature (PRO-56) and is **not yet implemented**. Currently, these words are not stripped during normalization, which means `Ancient Dusk - Clean.wav` and `Ancient Dusk - Full Mix.wav` may not automatically group together unless the core song name matches exactly via Phase 1 or scores above 0.85 in Phase 2.
:::
