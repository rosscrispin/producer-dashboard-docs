...
Every track in your library has a **stage** and a **workflow state** that together tell you where it is in the production pipeline. Stages represent the specific phase of work (Idea, Recording, Editing, Mixing, Mastering, Ready to Post, Published), while workflow states indicate the overall status (planned, active, on hold, completed).
...
Stages are shown as circular progress indicators that fill based on where a track stands in its workflow. When you select a track in the grid, the overview widget in the activity panel displays a larger version of this indicator with the stage name clearly labeled.

**Stage colors follow a consistent pattern across the app:**

- **Ideas & Planning** — light gray or muted blue
- **Recording & Capture** — warm orange or amber
- **Editing & Arrangement** — purple or violet
- **Mixing** — deep blue
- **Mastering** — gold or yellow
- **Ready to Post** — green
- **Published** — teal or cyan

When you open the track detail modal, you can see the full list of available stages and change the current stage with a single click. The stage history is also logged, so you can track when a track moved from one stage to another.
...
For example, imagine you want to focus only on tracks that are actively being mixed or mastered. Simply open the filter panel, select Mixing and Mastering from the stages list, and your grid updates instantly to show only those tracks. The filter count badge in the toolbar tells you how many active filters are applied, so you always know whether you're seeing your full library or a subset.
